...
class PretokDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        assert len(shard_filenames) > 0, "No files found!"
        while True:
            ...
            for shard in shard_filenames:
                ...
                pause_tokens = False
                if pause_tokens:
                    pause_chunk_len = (self.max_seq_len // 2) - 1
                    num_batches = len(m) // pause_chunk_len
                    num_batches -= 1  # drop the last partial batch
                    assert num_batches > 0, "this shard is way too small? investigate."
                    ixs = list(range(num_batches))
                    rng.shuffle(ixs)
                    for ix in ixs:
                        start = ix * pause_chunk_len
                        end = start + pause_chunk_len + 1
                        # calling .astype will copy the data into a new numpy array, now in RAM
                        chunk = (m[start:end]).astype(np.int64)
                        x, y = chunk, chunk
                        positions = np.arange(0, len(chunk), 1)

                        x = np.insert(x, positions, 3)
                        y = np.insert(y, positions, -1)
